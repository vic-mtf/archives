...
export default function ArchiveManagementContent () {
    ...
    const id = useSelector(store => store.data.navigation.archiveManagement.selectedElements[0]);
    const doc = useSelector(store => store.data?.docs?.find(doc => doc?._id === id));

    return (
        <>  
            {doc ? <ListData doc={doc}/> : <EmptyScreen/>}
        </>
    )
};
