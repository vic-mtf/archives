...
const TextFieldController = forwardRef((props, ref) => {
  ...
  useLayoutEffect(() => {
    if (ref)
      ref.current = !values.value.trim() || values.error ? null : values.value;
    if (!values.value.trim() && !values.error)
      setValues({ ...values, error: true });
  }, [checking, values, ref]);

  return (
    <FormControl color={color} type={values.type} {...otherPros}>
        ...
        endAdornment={
          <React.Fragment>
                ...
                <IconButton
                  ...
                  onClick={
                    typeof onToggleShowPassword === "function"
                      ? () =>
                          onToggleShowPassword(!values.showPassword, () =>
                            setValues({
                              ...values,
                              showPassword: !values.showPassword,
                            })
            ...
            {checking && icon && (
              <InputAdornment position='end'>
                {color === "error" ? (
                  <ErrorRoundedIcon fontSize='small' color={color} />
                ) : (
